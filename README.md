...
| `lowfs.h / lowfs.cpp` | Layer 3 | Low-level FS – directory management, path resolution, symlink following |
...
- Max files: 1024
- Each i-node occupies one complete block 
- i-node has 10 direct blocks, 1 single indirect, 1 double indirect
- i-node includes `link_count` for hard link support
...
cd <dir>                     Change current directory (cd .. to go up)
...
ln -s <target> <link>        Create a soft (symbolic) link
...
- Data is deleted only when all hard links are removed

**Soft link** — a pointer that stores the target path:
```
ln -s original.txt softlink.txt
```
- If the original is deleted, the link becomes broken
- `open` follows symlinks automatically
...
/root
  /user1
    /a
      /b
        /c
          file.txt   ← any depth supported
  /user2
    ...
```

Use `cd` to navigate and relative paths in all commands.
